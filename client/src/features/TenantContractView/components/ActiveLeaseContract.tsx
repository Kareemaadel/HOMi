                                    ...
                                    <>
                                        ...
                                        <div className="pdf-section">
                                            ...
                                            <div className="pdf-clause-list">
                                                <div className="pdf-clause-item">١. {previewLang === 'en' ? `Description: The property is located at ${contract.propertyAddress}. It consists of the unit specified (${localizedPreview.propertyType}).` : `الوصف: يقع العقار في ${contract.propertyAddress}. ويتكون من الوحدة المحددة (${localizedPreview.propertyType}).`}</div>
                                                <div className="pdf-clause-item">٢. {previewLang === 'en' ? `Duration: The contract starts on ${localizedPreview.startDate} and has a duration of ${localizedPreview.duration}.` : `المدة: يبدأ العقد في ${localizedPreview.startDate} ومدته ${localizedPreview.duration}.`}</div>
                                                <div className="pdf-clause-item">٣. {previewLang === 'en' ? `Value: The monthly rent is $${localizedPreview.amount}. It must be paid in advance at the beginning of each month.` : `القيمة: مبلغ الإيجار الشهرى هو ${localizedPreview.amount} جنية مصري. يجب دفعه مقدماً في بداية كل شهر.`}</div>
                                                <div className="pdf-clause-item">٤. {previewLang === 'en' ? `Deposit: A security deposit of $${localizedPreview.deposit} is paid. Refundable if no damages or unpaid bills.` : `التأمين: يتم دفع تأمين قدره ${localizedPreview.deposit} جنية مصري. يسترد إذا لم تكن هناك أضرار أو فواتير.`}</div>
                                                <div className="pdf-clause-item">٥. {previewLang === 'en' ? `Late Payment: A late fee of $${localizedPreview.lateFee} applies if payment is delayed more than 5 days.` : `التأخير: تطبق غرامة ${localizedPreview.lateFee} جنية مصري في حال التأخر عن الدفع لأكثر من ٥ أيام.`}</div>
                                                <div className="pdf-clause-item">٦. {previewLang === 'en' ? 'No Subleasing: Lessee cannot sublease or make changes without written consent.' : 'التنازل: لا يجوز للمستأجر التنازل عن الإيجار أو تغيير العقار دون موافقة.'}</div>
                                                <div className="pdf-clause-item">٧. {previewLang === 'en' ? 'Use: Property must be used for residential purposes only. Any other use terminates contract.' : 'الاستخدام: يستخدم العقار للسكن فقط. أي استخدام آخر ينهي العقد تلقائياً.'}</div>
                                                <div className="pdf-clause-item">٨. {previewLang === 'en' ? 'Expenses: Lessee expenses (decorations/improvements) are not reimbursable by lessor.' : 'المصاريف: مصاريف المستأجر (تحسينات/ديكور) لا يستردها وتصبح جزءاً من العقار.'}</div>
                                                <div className="pdf-clause-item">٩. {previewLang === 'en' ? 'Condition: Lessee must return property in original condition. Liable for negligence.' : 'الحالة: يجب إعادة العقار بحالته الأصلية. المستأجر مسؤول عن أي إهمال.'}</div>
                                                <div className="pdf-clause-item">١٠. {previewLang === 'en' ? 'Eviction: Lessee must vacate at end of term. Delay results in illegal occupation.' : 'الإخلاء: يجب الإخلاء عند انتهاء العقد. التأخير يعتبر شغلاً غير قانوني.'}</div>
                                                <div className="pdf-clause-item">١١. {previewLang === 'en' ? 'Utilities: Lessee is responsible for water, electricity, gas, and internet bills.' : 'المرافق: المستأجر مسؤول عن دفع فواتير الكهرباء والمياه والغاز والإنترنت.'}</div>
                                                <div className="pdf-clause-item">١٢. {previewLang === 'en' ? 'Termination: Early termination requires one month notice or one month rent penalty.' : 'الإنهاء: يتطلب الإنهاء المبكر إخطاراً قبل شهر أو دفع إيجار شهر غرامة.'}</div>
                                                <div className="pdf-clause-item">١٣. {previewLang === 'en' ? 'Correspondence: Addresses in contract are valid for all legal notices.' : 'المراسلات: العناوين المذكورة صحيحة لجميع الإخطارات القانونية والمراسلات.'}</div>
                                                <div className="pdf-clause-item">١٤. {previewLang === 'en' ? 'Jurisdiction: Digital copies provided to both parties. Subject to local courts.' : 'الاختصاص: نسختان رقميتان للطرفين. يخضع العقد للمحاكم المحلية.'}</div>
                                            </div>
                                        </div>

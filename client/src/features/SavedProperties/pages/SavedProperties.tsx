...
// Extracted mock data outside the component to keep it clean
...
const SavedProperties: React.FC = () => {
    ...
    // Convert mock data to state so we can clear it and trigger the empty state
    const [savedItems, setSavedItems] = useState(MOCK_SAVED_ITEMS);

    // Business Logic: Stats Calculation
    const stats = useMemo(() => {
        if (savedItems.length === 0) return { total: 0, avg: 0 };
        const total = savedItems.reduce((acc, curr) => acc + curr.price, 0);
        const avg = total / savedItems.length;
        return { total, avg };
    }, [savedItems]);
        ...
        <div className="dashboard-container">
            ...
            <div className="main-wrapper">
                ...
                <main className="saved-content">
                    ...
                    {/* 2. Properties Grid or Empty State */}
                    <div className="properties-grid-wrapper">
                        {savedItems.length > 0 ? (
                            ...
                            <div className="empty-saved-state">
                                {/* Custom Bot Video acting like a GIF */}
                                <video 
                                ...
                                <button className="btn-browse" onClick={() => navigate('/browse-properties')}>
                                    Browse Properties
                                </button>
                            </div>
                        )}
